...
// Get the client-side URL and key; allow localStorage overrides only if explicitly enabled
const getCredentials = () => {
  const allowLocalOverrides = (process.env.NEXT_PUBLIC_SUPABASE_ALLOW_LOCAL_OVERRIDES || '').toLowerCase() === 'true';

  if (allowLocalOverrides && typeof window !== 'undefined') {
    try {
  ...
  // Fallback to environment variables (default and recommended)
...
const initializeClient = () => {
  ...
  try {
    ...
    try { console.debug('[supabaseClient] Initialized', { url, source }); } catch {}
    ...
    (async () => {
      try {
        ...
        if (error) {
          console.error('Supabase client initialization test failed:', error);
        } else {
